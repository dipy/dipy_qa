...
class AppendTextFlow(Workflow):
    ...
    def run(self, input_files, input_file1, text_to_append='dipy', out_dir='',
        ...
        out_file : string, optional
            Name of the result file to be saved. (default '1.out')
        ...
        for in_file, in_file2, out_ in io_it:
            print("Hello: " + in_file + " " +  in_file2 + " " + out_)
            #pass

        all_text = []
        for in_file, in_file1, out_file in io_it:

            #print(in_file + ' ' + in_file1)

            #print(out_file)
            f = open(in_file, 'r')
            f1 = open(in_file1, 'r')
            all_text.extend(f.readlines())
            all_text.extend(f1.readlines())
            all_text.extend([text_to_append])
            with open(out_file, 'w') as myfile:
                myfile.write(' '.join(all_text))
            f.close()
            f1.close()
